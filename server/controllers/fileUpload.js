...
exports.imageUpload = async(request, respond) => {
    try{
        ...
        const uploadingImage = request.file;
        // uploadingImage = getDataUri(request.file);
        console.log(uploadingImage);

        // perform validation on files to supported or not
        const supportedTypes = ["jpg", "jpeg"];
        let fileType = request.file.originalname.split(".")[1];  // hitendra.pdf
        ...
        console.log("op");
        const uploadedImage = await uploadFiletoCloudinary(uploadingImage.path, "Hitendra");
